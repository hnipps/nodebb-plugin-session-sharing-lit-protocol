...
plugin.process = async ({ baseUrl, path, extraData, token }) => {
	const { verified, payload } = LitJsSdk.verifyJwt({ jwt: token });
    ...
    req.cookies[plugin.settings.cookieName].length
		...
		try {
			const uid = await plugin.process(
				Object.assign({}, req.body, {
					token: req.cookies[plugin.settings.cookieName],
				})
			);
			winston.verbose(
        ...
        req.originalUrl.replace(nconf.get('relative_path'), '');
			...
			case 'payload-invalid':
				...
				break;
			case 'token-invalid':
				winston.warn('[session-sharing] The passed-in token was invalid');
				break;
			case 'no-match':
				winston.info(
